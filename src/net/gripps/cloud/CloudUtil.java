...
public class CloudUtil {
    ...
    public static Random uniformRand = new Random();

    // Optional fixed seed for reproducible experiments. Negative means "not fixed".
    public static long random_seed = -1L;
    ...
    public static long genLong(long min, long max) {
        ...
        return min + (long) (uniformRand.nextDouble() * (max - min + 1));

    }

    public static int genInt(int  min, int max) {

        return min + (int) (uniformRand.nextDouble() * (max - min + 1));
    ...
    public void initialize(String propName){
        try{
            ...
            String seedStr = CloudUtil.prop.getProperty("random_seed");
            if (seedStr != null && seedStr.trim().length() > 0) {
                CloudUtil.random_seed = Long.valueOf(seedStr.trim());
                CloudUtil.rDataGen.reSeed(CloudUtil.random_seed);
                CloudUtil.uniformRand = new Random(CloudUtil.random_seed);
            } else {
                CloudUtil.random_seed = -1L;
                CloudUtil.uniformRand = new Random();
            }
