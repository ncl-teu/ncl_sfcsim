 ...
 */
public class HEFTD_VNFAlgorithm extends BaseVNFSchedulingAlgorithm {
    ...
    public HEFTD_VNFAlgorithm(CloudEnvironment env, SFC sfc) {
        super(env, sfc);
    }
    ...
    /**
     * HEFTD simplification:
     * disable cross-host image sharing.
     * same-DC same-host reuse (zero transfer) is still allowed.
     */
    private DownloadPlan findBestDownloadPlan(VNF vnf, VCPU targetVcpu) {
        ...
        // Keep only local reuse candidates (same DC + same physical host).
        ...
        while (vmIterator.hasNext()) {
            ...
            if (candidate == null || candidate.sourceHost == null || candidate.targetHost == null) {
                continue;
            }

            boolean isSamePhysicalHost =
                    candidate.sourceHost.getDcID().longValue() == candidate.targetHost.getDcID().longValue()
                            && candidate.sourceHost.getMachineID() == candidate.targetHost.getMachineID();
            if (!isSamePhysicalHost) {
                continue;
            }

            if (this.isTraceEnabled()) {
                this.trace("[HEFTD-DL-LOCAL-CAND]",
                        this.formatVNF(vnf)
                                + " target=" + this.formatVCPU(targetVcpu)
                                + " sourceVM=" + (candidate.sourceVM == null ? "null" : candidate.sourceVM.getVMID())
